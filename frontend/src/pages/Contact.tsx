const contactMethods = [
  {
    ...
    glow: 'group-hover:shadow-[0_0_30px_-8px_rgba(139,92,246,0.7)] group-hover:text-violet-300',
    icon: (
  ...
  },
  {
    ...
    glow: 'group-hover:shadow-[0_0_30px_-8px_rgba(255,255,255,0.35)] group-hover:text-white',
    icon: (
  ...
  },
  {
    ...
    glow: 'group-hover:shadow-[0_0_30px_-8px_rgba(34,211,238,0.6)] group-hover:text-cyan-300',
    icon: (
  ...
  return (
    <div className="bg-mesh relative">
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-16 sm:px-6">
        <h1 className="text-center text-4xl font-bold tracking-tight text-white">Contact</h1>

        <div className="glass mt-10 rounded-2xl p-6 sm:p-10">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
            {contactMethods.map((method) => (
              <a
                ...
                rel="noreferrer"
                className="group flex flex-col items-center rounded-xl border border-white/10 bg-white/[0.02] p-5 text-center transition-all hover:-translate-y-1 hover:border-white/20"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  className={`h-10 w-10 rounded-full p-2 text-slate-300 transition-all group-hover:scale-110 ${method.glow}`}
                >
                  {method.icon}
                </svg>
                <h2 className="mt-3 font-semibold text-white">{method.label}</h2>
                <span className="mt-1 break-words text-sm text-slate-500 group-hover:text-slate-300">
                  {method.value}
                </span>
              </a>
            ))}
          ...
          <p className="mt-8 border-t border-white/10 pt-6 text-center text-sm text-slate-500">
            N'hésitez pas à me contacter pour discuter de projets data, collaborations ou opportunités
            professionnelles.
          </p>
        </div>
      </div>
